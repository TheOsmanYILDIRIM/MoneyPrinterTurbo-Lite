...
def _render_video_settings(panel, params):
    ...
    with panel:
        with st.container(border=True):
            ...
            # Base resolution: drives both Pexels/Pixabay material selection and
            # the final composition size. Lower values produce smaller files.
            video_resolution_options = ["1080p", "720p", "540p", "480p"]
            saved_video_resolution = config.app.get("video_resolution", "1080p")
            if saved_video_resolution not in video_resolution_options:
                saved_video_resolution = "1080p"
            selected_video_resolution = stable_selectbox(
                tr("Base Resolution"),
                options=video_resolution_options,
                default_value=saved_video_resolution,
                key="video_resolution_select",
            )
            _set_runtime_config("app", "video_resolution", selected_video_resolution)

            # FFmpeg compression strength (CRF) for software encoders.
            saved_video_crf = config.app.get("video_crf", "")
            saved_video_crf_int = (
                int(saved_video_crf)
                if str(saved_video_crf).isdigit()
                else 23
            )
            selected_video_crf = st.slider(
                tr("FFmpeg Compression (CRF)"),
                min_value=0,
                max_value=51,
                value=saved_video_crf_int,
                step=1,
                help=tr("FFmpeg Compression Help"),
            )
            if selected_video_crf == 23:
                # 23 是 libx264 默认值，无须落盘，让配置表达“跟随编码器默认”。
                _delete_runtime_config("app", "video_crf")
            else:
                _set_runtime_config("app", "video_crf", selected_video_crf)
