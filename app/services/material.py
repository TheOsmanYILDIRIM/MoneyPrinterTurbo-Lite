...
def search_videos_pexels(
    ...
    try:
        ...
        for v in videos:
            ...
            # Pick the rendition that best fits the configured base resolution.
            # Earlier logic required an exact width/height match, which left very
            # few (sometimes a single) candidates for non-standard targets such
            # as 720x1280, so the final video reused one clip over and over.
            # Now we accept any aspect-matching rendition and choose the one
            # closest to the target resolution; the composition resizes it.
            best_video = None
            best_score = None
            for video in video_files:
                ...
                if not _matches_video_aspect(w, h, aspect):
                    continue
                # Prefer renditions that are at least the target size, then the
                # closest one; otherwise fall back to the largest available.
                if w >= target_width and h >= target_height:
                    score = (w - target_width) + (h - target_height)
                else:
                    score = 100000 - (w + h)
            ...
            item.source_info = {
                ...
                "rendition": {
                    "id": (
                        str(best_video.get("id"))
                        if best_video.get("id") is not None
                        else None
                    ),
                    "width": w,
                    "height": h,
