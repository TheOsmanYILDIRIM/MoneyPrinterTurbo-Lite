...
def _get_configured_video_crf() -> int | None:
    """
    读取用户配置的 FFmpeg 压缩强度（CRF）。

    CRF 仅作用于 libx264/libx265 这类软件编码器：数值越大压缩越强、文件越小，
    画质越低。硬件编码器（NVENC 等）使用不同的码率策略，这里不注入 CRF，
    避免把软件编码参数误传给硬件编码器导致编码失败。
    """
    raw = config.app.get("video_crf", None)
    if raw in (None, ""):
        return None
    try:
        crf = int(raw)
    except (TypeError, ValueError):
        logger.warning(f"invalid video_crf configured: {raw!r}, ignore it")
        return None
    return max(0, min(51, crf))


def _apply_compression_kwargs(kwargs: dict) -> None:
    """为 libx264/libx265 编码注入 CRF 压缩参数（若已配置）。"""
    crf = _get_configured_video_crf()
    if crf is not None and "crf" not in kwargs:
        kwargs["crf"] = crf
...
def _fallback_write_videofile(clip, output_file: str, failed_codec: str, reason: str, **kwargs):
    ...
    _apply_compression_kwargs(kwargs)
    ...
    for material in materials:
        ...
        try:
            ...
            if ext in const.FILE_TYPE_IMAGES:
                ...
                preprocess_kwargs = {}
                _apply_compression_kwargs(preprocess_kwargs)
                final_clip.write_videofile(video_file, fps=30, logger=None, preset="ultrafast", **preprocess_kwargs)
                close_clip(clip)
